...
export function ProductCard({
  ...
  return (
    <article className="glass-panel glass-soft group relative overflow-hidden rounded-[28px] border border-white/15 bg-white/8 p-3 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-emerald-200/20 hover:shadow-glow">
      ...
      <div className="absolute -right-8 top-8 h-24 w-24 rounded-full bg-white/10 blur-2xl transition duration-500 group-hover:scale-125" />
      <div className="relative overflow-hidden rounded-[24px] border border-white/10 bg-black/5">
        <img
          ...
          loading="lazy"
        />
        <div className="pointer-events-none absolute inset-x-3 top-3 h-14 rounded-full bg-white/10 blur-xl" />
        {product.discount ? (
      ...
      <div className="relative space-y-4 px-1 pb-1 pt-4">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="premium-ring shrink-0 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-[var(--text-secondary)]">
        ...
        <div className="flex items-end justify-between gap-3">
          ...
          <button
            ...
            className="premium-ring inline-flex items-center gap-2 rounded-full border border-emerald-200/20 bg-gradient-to-r from-emerald-300 via-emerald-200 to-white px-4 py-3 text-sm font-bold text-emerald-950 shadow-lg transition hover:scale-[1.02] active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <ShoppingBag size={16} />
